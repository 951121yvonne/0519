...
// 【修改點】新增 thumbs_down 的表情與文字對應，並將 thumbs_up 改為「繼續」提示
...
const BUF = 10, HOLD = 400, CD = 3; // 判定時間 (0.4s)
...
function drawMask(cx, cy, p) {
    ...
    if (p > .35) {
        ...
        [cx - 24, cx + 24].forEach(ex => {
            g.fillStyle = '#1A0000'; g.beginPath(); g.ellipse(ex, cy - 16, 13 * q, 8 * q, 0, 0, Math.PI * 2); g.fill();
            g.fillStyle = '#DDD'; g.beginPath(); g.arc(ex + 3, cy - 20, 4 * q, 0, Math.PI * 2); g.fill();
        });
        g.strokeStyle = '#1A0000'; g.lineWidth = 4; g.lineCap = 'round'; g.beginPath();
        g.moveTo(cx - 28 * q, cy + 26); g.quadraticCurveTo(cx, cy + 52 * q, cx + 28 * q, cy + 26); g.stroke();
        if (q > .5) {
            const tp = (q - .5) / .5; g.fillStyle = 'rgba(90,140,255,.85)';
            [cx - 27, cx + 27].forEach(tx => { g.beginPath(); g.ellipse(tx, cy - 2 + 28 * tp, 4, 13 * tp, 0, 0, Math.PI * 2); g.fill(); });
        }
        [[-1, cx - r + 15], [1, cx + r - 15]].forEach(([d, hx]) => {
            g.fillStyle = '#7A0000'; g.strokeStyle = '#FF3300'; g.lineWidth = 2; g.beginPath();
            g.moveTo(hx - 10 * d, cy - r * .78); g.lineTo(hx, cy - r * 1.22 * q); g.lineTo(hx + 10 * d, cy - r * .78); g.closePath(); g.fill(); g.stroke();
        });
        g.strokeStyle = '#CC0000'; g.lineWidth = 3; g.lineCap = 'round';
        [[cx - 50, cy], [cx + 50, cy]].forEach(([ex, ey]) => {
            const s = 7 * q; g.beginPath(); g.moveTo(ex - s, ey - s); g.lineTo(ex + s, ey + s); g.stroke();
            g.beginPath(); g.moveTo(ex + s, ey - s); g.lineTo(ex - s, ey + s); g.stroke();
        });
        g.strokeStyle = 'rgba(180,0,0,.5)'; g.lineWidth = 1.5; g.beginPath(); g.ellipse(cx, cy, r * 1.12, r * 1.28, 0, 0, Math.PI * 2); g.stroke();
    }
    g.restore();
}
...
function dIdle() {
    ...
    if (!lm) {
        ...
        smT(!isValid ? '⚠️ 這是功能鍵，請比出猜拳手勢' : pct < 1 ? '保持手勢，即將開始...' : 'GO!', W / 2, H - 44, 13, 'rgba(255,255,255,.7)');
    } else {
...
function dCountdown() {
    const el = Date.now() - stAt; skel(); scoreHUD();
...
function dReveal() {
    const el = Date.now() - stAt; const cpuA = Math.min(1, Math.max(0, (el - 400) / 500));
    ...
    boldT('你', W / 4, 36, 20, '#AAD4FF'); boldT('電腦', W * 3 / 4, 36, 20, '#FFAAAA');
...
function dWin() {
    drawP(); scoreHUD(); const el = Date.now() - stAt, pulse = 1 + .07 * Math.sin(el / 170);
...
function dLose() {
    const el = Date.now() - stAt; maskP = Math.min(1, el / 700);
    g.fillStyle = `rgba(140,0,0,${maskP * .35})`; g.fillRect(0, 0, W, H);
    drawMask(W * .72, H * .42, maskP); scoreHUD();
...
function dDraw() {
    const el = Date.now() - stAt, pulse = 1 + .06 * Math.sin(el / 160); scoreHUD();
    g.fillStyle = 'rgba(0,0,0,.65)'; g.fillRect(0, 0, W, 88);
...
// 【核心修改點】重新定義選單視覺元件，完全支持 👍/👎 提示與進度條繪製
function dMenu() {
    ...
    g.font = '14px Arial'; g.textAlign = 'center'; g.textBaseline = 'middle'; g.fillStyle = 'rgba(255,255,255,.5)';
    ...
    g.font = '13px Arial'; g.textAlign = 'center'; g.textBaseline = 'middle'; g.fillStyle = 'rgba(255,255,255,.45)';
    g.fillText('💡 比出 👍 🎮 繼續  ·  比出 👎 🏠 結束 (不分左右手)', W / 2, H / 2 + 106); g.restore();

    // 【加分項：動態手勢進度條】
    if (st === 'menu' && (stable === 'thumbs_up' || stable === 'thumbs_down')) {
        const pct = menuHoldT ? Math.min(1, (Date.now() - menuHoldT) / HOLD) : 0;
        const isContinue = stable === 'thumbs_up';
        const col = isContinue ? '#00FF88' : '#FF4444'; // 繼續顯示綠色，結束顯示紅色
        ...
        const txt = isContinue ? '🎮 準備繼續...' : '🏠 準備結束...';
...
function dEnded() {
    ...
    g.save(); g.font = 'bold 20px Arial'; g.textAlign = 'center'; g.textBaseline = 'middle'; g.fillStyle = 'rgba(255,255,255,.65)';
...
function update() {
    ...
    // 【核心修改點】重新編寫選單狀態的觸發邏輯，直接判斷 👍 與 👎，徹底捨棄不穩定的左右手識別
    if (st === 'menu') {
        if (stable === 'thumbs_up' || stable === 'thumbs_down') {
            if (!menuHoldT) menuHoldT = now; // 鎖定啟動時間點
            if (now - menuHoldT >= HOLD) {   // 超過緩衝時間（0.4秒）即觸發
                if (stable === 'thumbs_up') startGame();
                else if (stable === 'thumbs_down') enter('ended');
                menuHoldT = null;
            }
        } else {
            menuHoldT = null; // 手勢中斷則重置計時器
        }
    }
    ...
    if (st === 'countdown') {
        ...
        if (el >= CD * 1000) {
            ...
            cG = PICKS[Math.random() * 3 | 0]; enter('reveal');
        }
    }
    if (st === 'reveal' && el > 1500) {
        const res = pG === cG ? 'draw' : BEATS[pG] === cG ? 'win' : 'lose';
        if (res === 'win') score.w++; else if (res === 'lose') score.l++; else score.d++;
        enter(res); maskP = 0; if (res === 'win') startFW();
    }
...
function onClk(e) {
    ...
    const bw = 132, bh = 52, by = H / 2 + 24;
    if (cx >= W / 2 + 8 && cx <= W / 2 + 8 + bw && cy >= by && cy <= by + bh) startGame(); // 點擊右鍵：繼續
    if (cx >= W / 2 - bw - 8 && cx <= W / 2 - 8 && cy >= by && cy <= by + bh) enter('ended'); // 點擊左鍵：結束
}

function startGame() {
    parts = []; maskP = 0; gBuf = []; stable = null;
    holdT = null; pG = null; cG = null; menuHoldT = null;
    stopFW(); enter('idle');
}
